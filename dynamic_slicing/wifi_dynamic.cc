...
// function to set the tx power (signal strength of wifi)
void set_tx_power()
{
  for (int i = 0; i < nStaA; i++)
  {
                      ...
                      NodeContainer staNodes, NodeContainer apNode)
{
  ...
  if (probErrSumA < 0) eMMB_improved = 1;

  if (!eMMB && !eMMB_improved) // SLA KO in the last T && Pe in the last T > in previous T
  {
    channelWidthMulA = 2;
  }
  if (eMMB && eMMB_improved) // SLA OK in the last T && Pe in the last T < in previous T
  {
    channelWidthMulA = 1;
  }
  ...
  std::cout << "eMMB : " <<channelWidthA << ", " << channelNumberA <<
  ...
  if (nSliceB_off > nStaB/10) mMTC = 0;
  double probErrSumB = 0;
  for (int i = 0; i < nStaB; i++)
  {
    probErrSumB = probErrSumB + probErr[0][nStaA+i] - probErr[1][nStaA+i];
  }
  if (probErrSumB < 0) mMMTC_improved = 1;

  if (!mMTC && !mMMTC_improved) // SLA KO in the last T && Pe in the last T > in previous T
  {
  ...
  if (mMTC && mMMTC_improved) // SLA OK in the last T && Pe in the last T < in previous T
  {
  ...
  std::cout << "mMMTC : " << channelWidthB << ", " << channelNumberB <<
 ...
 URRLC = 1;
  ...
  if (probErrSumC < 0) URLLC_improved = 1;

  if ( URRLC && !URLLC_improved) // SLA KO in the last T && Pe in the last T > in previous T
  {
    //channelWidthMulC = 2;
    channelWidthMulC = 1;
  }
  if (URRLC && URLLC_improved) // SLA OK in the last T && Pe in the last T < in previous T
  {
    channelWidthMulC = 1;
  }
  ...
  std::cout << "URLLC: " << channelWidthC << ", " << channelNumberC <<
               ", " <<giC << ", " <<mcsC << ", " <<txPowerC << std::endl;
  // std::cout << "URLLC mcs: " << mcsC_min << ", " << mcsC_max << std::endl;
                      ...
                      NodeContainer staNodes, NodeContainer apNode)
{
  ...
  std::cout << "eMMB : " <<"channelWidthA :"<<channelWidthA << ", " <<"channelNumberA :" << channelNumberA <<
               ", " <<"giA :" << giA << ", " <<"mcsA :" <<mcsA << ", " <<"txPowerA :" <<txPowerA << std::endl;
  // std::cout << "eMMB mcs: " << mcsA_min << ", " << mcsA_max << std::endl;
...
std::cout << "mMMTC : " <<"channelWidthB :"<< channelWidthB << ", " <<"channelNumberB :"<< channelNumberB <<
               ", " <<"giB :"<< giB << ", " <<"mcsB :"<< mcsB << ", " <<"txPowerB :"<<txPowerB << std::endl;
  ...
  std::cout << "URLLC: " <<"channelWidthC :"<< channelWidthC << ", " <<"channelNumberC :"<< channelNumberC <<
               ", " <<"giC :"<<giC << ", " <<"mcsC :"<<mcsC << ", " <<" txPowerC :" <<txPowerC << std::endl;
  // std::cout << "URLLC mcs: " << mcsC_min << ", " << mcsC_max << std::endl;
